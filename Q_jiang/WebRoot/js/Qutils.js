	...
	Qutils.prototype = {
		...
		goPage: function(url, param, is_hide){
			...
			if (param && !$.isEmptyObject(param)){
				if (is_hide){
					// 将参数进行加密
					hide_str = Aes.Ctr.encrypt(JSON.stringify(param), _pas_key, 256);
				} else {
					var	key = "",
						val = "";
					for (key in param){
					...
					show_str = show_str.substr(0, show_str.lastIndexOf("&"));
					// 对参数进行加密
					show_str = "?" + Aes.Ctr.encrypt(show_str, _pas_key, 256);
				}
				sessionStorage[_param_key] = hide_str;
			} else {
				sessionStorage.removeItem(_param_key);
			}
			window.location.href = (url + show_str);
		},
		...
		getParam: function(key){
			...
			var param = {},
				session_param = sessionStorage[_param_key];
			// 判断页面是否有隐藏参数
			if (session_param){
				param = JSON.parse(Aes.Ctr.decrypt(session_param, _pas_key, 256));
			} else {
				var url = window.location.href;
				var	url_split = url.split(".html?")[1];
				var	url_dec = Aes.Ctr.decrypt(url_split, _pas_key, 256);
				if (url_dec){
					var arr = url_dec.split("&"),
		...
		setLStorage: function(key, val){
			// 保存
			window.localStorage[key] = Aes.Ctr.encrypt(val, _pas_key, 256);
		},
		...
		getLStorage: function(key){
			// 保存
			return Aes.Ctr.decrypt(window.localStorage[key], _pas_key, 256);
		},
		
		/**
		 * ajax 提交数据
		 */
		Qpost: function(param, success, is_async){
			ajaxs.type = "post";
			ajaxs.success = success;
			ajaxs.data = param;
			ajaxs.async = (typeof is_async === "undefined" || is_async === true) ? true : false;
			$.ajax(ajaxs);
		},
		/**
		 * ajax 查询数据
		 */
		Qget: function(param, success, is_async){
			ajaxs.type = "get";
			ajaxs.success = success;
			ajaxs.data = param;
			ajaxs.async = (typeof is_async === "undefined" || is_async === true) ? true : false;
